...
// Pomocna funkcia na vycistenie vstupneho bufferu
// Po scanf() mozu v bufferi zostat zbytocne znaky
// Tato funkcia ich odstrani az do konca riadku
void vycisti_vstup() {
    int znak;
    while ((znak = getchar()) != '\n' && znak != EOF) {
        // 1. Precitaj znak - getchar() - automaticky vymaze z bufferu
        // 2. Pokracuj, kym nenarazis na koniec riadku alebo EOF
        // prazdne telo cyklu - len citame a ignorujeme znaky
    }
}
...
int validuj_prvky_matice(const char* nazov_suboru, int rozmer_matice) {
    ...
    if (preskumane_prvky == ocakavany_pocet) {
        ...
        printf("    [CHYBA] Pocet preskumaných prvkov (%d) neodpoveda ocakávanému poètu (%d)\n", preskumane_prvky, ocakavany_pocet);
        return -1;
    }
